...
import { RegionForm, RegionSchema } from "../../schemas/RegionSchema";
...
const Regions = () => {
  ...
  const [formRecord, setFormRecord] = useState<FormRecord<Region>>({
    record: null,
    isCreate: true,
    loading: false,
    showModal: false,
  ...
  const [deleteRecord, setDeleteRecord] = useState<DeleteRecord<Region>>({
    record: null,
    loading: false,
    showModal: false,
  });
  ...
  const handleEditRecord = (values: Region) => {
  ...
  const onSubmit = (values: RegionForm) => {
  ...
  const handleDelete = (record: Region) => {
  ...
  const columns = [
    {
      ...
      dataIndex: "s_no",
      key: "s_no",
      render: (value: string, record: Region, index: number) => {
    ...
    },
    {
      ...
      dataIndex: "action",
      key: "action",
      render: (value: string, record: Region) => {
        return (
        ...
        <div className="flex flex-col gap-4">
          ...
          <Dialog isOpen={formRecord.showModal} onClose={handleCloseForm}>
            <h4 className="mb-4 text-center">
              {formRecord.isCreate ? "Create Region" : "Update Region"}
            </h4>
