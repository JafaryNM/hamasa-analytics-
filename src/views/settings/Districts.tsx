...
import { DistrictForm, DistrictSchema } from "../../schemas/DistrictSchema";
...
const Districts = () => {
  ...
  const [formRecord, setFormRecord] = useState<FormRecord<District>>({
    record: null,
    isCreate: true,
    loading: false,
    showModal: false,
  ...
  const [deleteRecord, setDeleteRecord] = useState<DeleteRecord<District>>({
    record: null,
    loading: false,
    showModal: false,
  });
  ...
  useEffect(() => {
    ...
    request
      .then((res) => {
        ...
        setRegions(
          results != null
            ? results.map((reg: Region) => {
                return { label: reg.name, value: reg.uuid };
              })
            : []
        );
      })
  ...
  const handleEditRecord = (values: District) => {
    reset({
      name: values.name,
      region: { label: values.region.name, value: values.region.uuid },
    });
    setFormRecord({
      ...formRecord,
      record: values,
  ...
  const onSubmit = (values: DistrictForm) => {
  ...
  const handleDelete = (record: District) => {
  ...
  const columns = [
    {
      ...
      dataIndex: "s_no",
      key: "s_no",
      render: (value: string, record: District, index: number) => {
    ...
    },
    {
      ...
      dataIndex: "region",
      key: "region",
      render: (value: Region) => value.name,
    },
    {
      title: "Created At",
      dataIndex: "createdAt",
    ...
    },
    {
      ...
      dataIndex: "action",
      key: "action",
      render: (value: string, record: District) => {
        return (
        ...
        <div className="flex flex-col gap-4">
          ...
          <Dialog isOpen={formRecord.showModal} onClose={handleCloseForm}>
            <h4 className="mb-4 text-center">
              {formRecord.isCreate ? "Create District" : "Update District"}
            </h4>
