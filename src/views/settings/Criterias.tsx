...
import { CriteriaForm, CriteriaSchema } from "../../schemas/CriteriaSchema";
...
const Criterias = () => {
  ...
  const [formRecord, setFormRecord] = useState<FormRecord<Criteria>>({
    record: null,
    isCreate: true,
    loading: false,
    showModal: false,
  ...
  const [deleteRecord, setDeleteRecord] = useState<DeleteRecord<Criteria>>({
    record: null,
    loading: false,
    showModal: false,
  });
  ...
  const resetField = () => {
    reset({ name: "", minScore: "", maxScore: "", description: "" });
  };
  ...
  const handleEditRecord = (values: Criteria) => {
    reset({
      name: values.name,
      minScore: values.minScore.toString(),
      maxScore: values.maxScore.toString(),
  ...
  const handleDelete = (record: Criteria) => {
  ...
  const columns = [
    {
      ...
      dataIndex: "s_no",
      key: "s_no",
      render: (value: string, record: Criteria, index: number) => {
    ...
    },
    {
      ...
      dataIndex: "action",
      key: "action",
      render: (value: string, record: Criteria) => {
        return (
        ...
        <div className="flex flex-col gap-4">
          ...
          <Dialog isOpen={formRecord.showModal} onClose={handleCloseForm}>
            <h4 className="mb-4 text-center">
              {formRecord.isCreate ? "Create Criterias" : "Update Criterias"}
            </h4>
            <Form onSubmit={handleSubmit(onSubmit)}>
              ...
              >
                <Controller
                  ...
                  render={({ field }) => (
                    <Input
                      type="number"
                      placeholder="Maximum Score in this criteria"
                      autoComplete="off"
                      {...field}
                    />
                  )}
              ...
              <FormItem
                label="Minimum Score"
                invalid={Boolean(errors.minScore)}
              ...
              >
                <Controller
                  ...
                  render={({ field }) => (
                    <Input
                      type="number"
                      placeholder="Minimum Score in this criteria"
                      autoComplete="off"
                      {...field}
                    />
                  )}
