...
import useMediaChannels from "@/hooks/useMediaChannels";
...
import {
  MediaChannelForm,
  MediaChannelSchema,
} from "../../schemas/MediaChannelSchema";
...
const MediaChannels = () => {
  ...
  const [formRecord, setFormRecord] = useState<FormRecord<MediaChannel>>({
    record: null,
    isCreate: true,
    loading: false,
    showModal: false,
  ...
  const [deleteRecord, setDeleteRecord] = useState<DeleteRecord<MediaChannel>>({
    record: null,
    loading: false,
    showModal: false,
  });
  ...
  const handleEditRecord = (values: MediaChannel) => {
  ...
  const onSubmit = (values: MediaChannelForm) => {
  ...
  const handleDelete = (record: MediaChannel) => {
  ...
  const columns = [
    {
      ...
      dataIndex: "s_no",
      key: "s_no",
      render: (value: string, record: MediaChannel, index: number) => {
    ...
    },
    {
      ...
      dataIndex: "action",
      key: "action",
      render: (value: string, record: MediaChannel) => {
        return (
        ...
        <div className="flex flex-col gap-4">
          ...
          <Dialog isOpen={formRecord.showModal} onClose={handleCloseForm}>
            <h4 className="mb-4 text-center">
              {formRecord.isCreate ? "Create Media Type" : "Update Media Type"}
            </h4>
